...
class PurchaseOrderLineItemSerializer(
    ...
    auto_pricing = serializers.BooleanField(
        ...
        help_text=_(
            'Automatically calculate purchase price based on supplier product data'
        ),
        default=True,
    )
    ...
    merge_items = serializers.BooleanField(
        label=_('Merge Items'),
        help_text=_(
            'Merge items with the same product, destination and target date into one line item'
        ),
        default=True,
        write_only=True,
    )
    ...
    ipn = serializers.CharField(
        source='part.part.IPN', read_only=True, label=_('Internal Product Number')
    )
...
class PurchaseOrderLineItemReceiveSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if base_part.trackable and Decimal(base_quantity) != int(base_quantity):
            raise ValidationError({
                'quantity': _(
                    'An integer quantity must be provided for trackable products'
                )
            })
