...
class PublicCreateSalesOrder(APIView):
    permission_classes = [permissions.AllowAny]

    def post(self, request):
        data = request.data

        # 1. Get or create customer (same as before)
        ...
        if customer_id:
            ...
            customer_fields = ['name', 'email', 'phone', 'address', 'currency']
            customer_data = {field: data.get(field) for field in customer_fields}
            if not customer_data['name'] or not customer_data['email']:
                return Response({'detail': 'Customer name and email are required.'}, status=status.HTTP_400_BAD_REQUEST)
            customer_data['is_customer'] = True
            serializer = CompanySerializer(data=customer_data)
            if serializer.is_valid():
                customer = serializer.save()
            else:
                return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

        # 2. Get items (list of products and quantities)
        items = data.get('items')
        if not items or not isinstance(items, list) or len(items) == 0:
            return Response({'detail': 'At least one item is required.'}, status=status.HTTP_400_BAD_REQUEST)

        now = datetime.datetime.now()
        reference = f"SO-{now.strftime('%Y%m%d%H%M%S')}"
        # 3. Create SalesOrder
        order = SalesOrder.objects.create(
            customer=customer,
            reference=reference,
            description=data.get('description', ''),
            status=10,  # Or your default status code for new orders
        )

        # 4. Create SalesOrderLineItems for each item
        for item in items:
...
class SalesOrderLineItemFilter(LineItemFilter):
    ...
    part = rest_filters.ModelChoiceFilter(
        queryset=Part.objects.all(), field_name='part', label=_('Product')
    )
...
class SalesOrderAllocationFilter(rest_filters.FilterSet):
    ...
    part = rest_filters.ModelChoiceFilter(
        queryset=Part.objects.all(), method='filter_part', label=_('Product')
    )
...
class FulfillmentSalesOrderListView(ListAPIView):
    """API endpoint for sales orders available for fulfillment."""
    ...
    def get_queryset(self):
        """Return sales orders that need fulfillment."""
...
class FulfillmentScanItemView(APIView):
    """API endpoint for scanning items during fulfillment."""
    
    permission_classes = [permissions.IsAuthenticated]
    
    def post(self, request):
        """Handle barcode scanning for fulfillment."""
        ...
        try:
            sales_order = SalesOrder.objects.get(pk=order_id)
        except SalesOrder.DoesNotExist:
            return Response({'error': 'Sales order not found'}, status=status.HTTP_404_NOT_FOUND)
        
        # Scan barcode using existing barcode plugins
        ...
        for plugin in plugins:
            try:
                ...
                if result and 'stockitem' in result:
                    scanned_item = StockItem.objects.get(pk=result['stockitem']['pk'])
                    break
            except Exception:
                continue
        
        if not scanned_item:
            # Try to find by barcode hash
            barcode_hash = hash_barcode(barcode)
            try:
                scanned_item = StockItem.objects.get(barcode_hash=barcode_hash)
            except StockItem.DoesNotExist:
                return Response({
                    'error': 'No stock item found for this barcode',
                    'barcode': barcode
                }, status=status.HTTP_404_NOT_FOUND)
        
        # Check if scanned item matches any line item in the order
        matching_lines = []
        if line_item_id:
            # Check specific line item
            try:
                line = SalesOrderLineItem.objects.get(pk=line_item_id, order=sales_order)
                if line.part == scanned_item.part:
                    matching_lines.append(line)
            except SalesOrderLineItem.DoesNotExist:
                pass
        else:
            # Check all line items
            matching_lines = list(sales_order.lines.filter(part=scanned_item.part))
        
        if not matching_lines:
            # Item doesn't match any line item - alert user
            return Response({
                'error': 'Scanned item does not match any items in this order',
                'scanned_item': {
                    'pk': scanned_item.pk,
                    'part_name': scanned_item.part.name,
                    'part_pk': scanned_item.part.pk,
                    'quantity': scanned_item.quantity,
                    'serial': scanned_item.serial,
                    'batch': scanned_item.batch
                },
                'order_parts': [
                    {
                        'pk': line.part.pk,
                        'name': line.part.name,
                        'line_item_pk': line.pk,
                        'quantity_needed': line.quantity - line.shipped
                    }
                    for line in sales_order.lines.all()
                    if line.quantity > line.shipped
                ],
                'allow_substitution': True
            }, status=status.HTTP_400_BAD_REQUEST)
        
        # Item matches - return success with fulfillment options
        line = matching_lines[0]
        quantity_needed = line.quantity - line.shipped
        available_quantity = min(scanned_item.quantity, quantity_needed)
        ...
        return Response({
            ...
            'line_item': {
                ...
                'quantity_shipped': line.shipped,
                'quantity_needed': quantity_needed
            },
            'suggested_quantity': available_quantity
        })


class FulfillmentSubstituteItemView(APIView):
    """API endpoint for substituting items during fulfillment."""
    ...
    def post(self, request):
        ...
        order_id = request.data.get('order_id')
        line_item_id = request.data.get('line_item_id')
        substitute_part_id = request.data.get('substitute_part_id')
        ...
        if not confirmation:
            return Response({
                'error': 'Substitution must be confirmed'
            }, status=status.HTTP_400_BAD_REQUEST)
        
        try:
            with transaction.atomic():
                sales_order = SalesOrder.objects.get(pk=order_id)
                line_item = SalesOrderLineItem.objects.get(pk=line_item_id, order=sales_order)
                substitute_part = Part.objects.get(pk=substitute_part_id)
                ...
                # Update the line item with substitute part
                original_part = line_item.part
                line_item.part = substitute_part
                line_item.notes = f"{line_item.notes}\nSubstituted {original_part.name} with {substitute_part.name}. {notes}".strip()
                line_item.save()
                
                # Reduce stock if stock item provided
                if stock_item and quantity:
                    if stock_item.quantity >= float(quantity):
                        stock_item.quantity -= float(quantity)
                        stock_item.save()
                        
                        # Update shipped quantity
                        line_item.shipped += float(quantity)
                        line_item.save()
                
                return Response({
                    'success': True,
                    'message': 'Item substituted successfully',
                    'line_item': {
                        'pk': line_item.pk,
                        'original_part': original_part.name,
                        'substitute_part': substitute_part.name,
                        'quantity_substituted': quantity
                    }
                })
...
class FulfillmentCompleteItemView(APIView):
    """API endpoint for completing fulfillment of an item."""
    
    permission_classes = [permissions.IsAuthenticated]
    
    def post(self, request):
        """Complete fulfillment of a scanned item."""
        stock_item_id = request.data.get('stock_item_id')
        line_item_id = request.data.get('line_item_id')
        quantity = request.data.get('quantity')
        ...
        try:
            with transaction.atomic():
                stock_item = StockItem.objects.get(pk=stock_item_id)
                line_item = SalesOrderLineItem.objects.get(pk=line_item_id)
                ...
                return Response({
                    ...
                    'remaining_stock': stock_item.quantity,
                    'line_item_shipped': line_item.shipped
                })
...
class FulfillmentMarkUnavailableView(APIView):
    """API endpoint for marking items as unavailable."""
    ...
    def post(self, request):
        ...
        try:
            ...
            # Add note about unavailability
            unavailable_note = f"Marked as unavailable during fulfillment. {notes}".strip()
